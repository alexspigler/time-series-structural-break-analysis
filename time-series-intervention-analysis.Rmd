...
**Constant Variance:** The residuals vs. fitted values plot shows residuals randomly distributed around y = 0, suggesting constant variance. This is supported by the Cook-Weisberg non-constant variance score test (p-value = 0.2911), which checks whether the residual variance scales with the fitted level — the natural heteroscedasticity concern for a trending series — and does not reject the null hypothesis of constant variance.
...
**Model Selection:** The ACF shows a significant spike at lag 1 that gradually tails off. The PACF similarly shows a significant spike at lag 1 that tails off. This pattern (both ACF and PACF showing initial spikes that decay) suggests an ARIMA(1,0,1) model is appropriate for the residuals. A simpler AR(1) model fits the residuals nearly as well, but both the AR and MA terms are statistically significant and the gradual tail-off in both the ACF and PACF supports the mixed ARMA(1,1) specification, so the MA(1) term is retained.
...
This model allows for a structural break in both the level and trend of sales at Week 88, and it captures autocorrelated, stationary errors. As a robustness check, re-estimating the regression coefficients jointly with the ARIMA(1,0,1) errors (generalized least squares rather than OLS) yields a very similar post-intervention slope, confirming the trend reversal is a genuine feature of the data and not an artifact of the OLS fit, whose standard errors the autocorrelation had invalidated.
